...
2. Server sets httpOnly `gcfieldlog_stub_user` = `{ userId, email, role }`. Same email → same `userId`. Cookie flags: `Path=/`, `SameSite=Lax`, `HttpOnly`, `Secure` on HTTPS. Production (`gcfieldlog.com` and `www.gcfieldlog.com`) also sets `Domain=gcfieldlog.com` so one Puller login is sent to both hosts — Vercel 308-redirects apex → www, and a host-only cookie would drop on that hop (Share, Refresh all, pin APIs). Localhost and `*.vercel.app` stay host-only (no Domain). Logout expires both the host-only leftover and the Domain cookie.
