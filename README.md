...
1. Login is a **document form POST** to `/api/session` with email + role (password ignored). The handler 303-redirects to `/jobs` (or `next`) and sets the httpOnly cookie on that document response so Chrome stores it. JSON POST still works for API/smoke; `fetch` Set-Cookie is not reliable in Chrome’s jar.
