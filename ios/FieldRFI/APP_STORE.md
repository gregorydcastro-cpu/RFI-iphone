...
- Panels: field tracker on G-Line Shop Test only. Build a panel and circuit list in the app. Check pulled vs not pulled so nothing is missed. Optional one-line note on a circuit. Optional fill from bundled **E-101 Rev A**. If that sample sheet is missing, it writes no circuits. Not a design log. Not a Procore sync. No Procore write. On this phone. No HTTP.
