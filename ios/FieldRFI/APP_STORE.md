...
2. **Screenshots** — iPhone 6.7" and 6.1" (plus iPad if the family stays 1,2): New RFI with a pin on a catalog sheet, Field problem, Material held list (G-Line Shop Test), Panel schedule (LP-1 on E-101 Rev A), Foreman inbox. Do not invent drawing numbers in the shots; use the bundled sample **E-101 Rev A**. Job name stays **G-Line Shop Test**. Use G-Line Shop Test and E-101 Rev A only.
...
- Panels: field tracker on G-Line Shop Test. Create a panel, add/edit circuits, check pulled vs not pulled so nothing is missed. Optional short change note on that circuit — not an RFI close, not a design log, not work_stopped. Optional fill from bundled **E-101 Rev A** (one circuit per visible plate fixture). If that sample sheet is missing, it writes no circuits. Procore import is a later stub only — disabled, no auth, no API, no write. On this phone. No HTTP.
