import { getJob, jobFromRequestId } from "@/lib/jobs";
...
/**
 * Read-only latest pack from `public.room_packs.pack_data`. Does not pull.
 */
async function handleStatus(input: {
  ...
  if (job && projectSlug && !requestBelongsToJob(job, requestId)) {
